...
namespace gazebo
{
  class RoombaCircle3 : public ModelPlugin
  {
  	public: void Load(physics::ModelPtr _parent, sdf::ElementPtr /*_sdf*/)
  	{
		...
		key = anim->CreateKeyFrame(70.65);
        	key->Translation(ignition::math::Vector3d(-5,0, 0));
        	key->Rotation(ignition::math::Quaterniond(0, 0, 0));

		key = anim->CreateKeyFrame(76.5375);
        	key->Translation(ignition::math::Vector3d(-4.33,2.5, 0));
        	key->Rotation(ignition::math::Quaterniond(0, 0, 0));
  ...
  GZ_REGISTER_MODEL_PLUGIN(RoombaCircle3)
}
